...
#' @examples
#' # Profile of the seizures of the placebo (left) and treatments (rigth) groups.
#' # Not run:
#' data(seizures)
#' n <- max(seizures$ind)
#' seizures1 <- seizures[seizures$period==1,]
#' mi<- length(seizures1$Y[seizures$ind==1])
#'
#' matplot(matrix(seizures1$Y[seizures1$trt==0],mi,
#' length(seizures1$Y[seizures1$trt==0])/mi),
#' type="l",xlab="Times", ylab="Seizures count",
#' main="",axes=F,cex.lab=1.6,cex.axis=1.6,col=1)
#' box()
#' axis(1,1:4,labels=c("2","4","6","8"))
#' axis(2)
#'
#' matplot(matrix(seizures1$Y[seizures1$trt==1],mi,
#' length(seizures1$Y[seizures1$trt==1])/mi),
#' type="l",xlab="Times", ylab="Seizures count",
#' main="",axes=F,cex.lab=1.6,cex.axis=1.6,col=1)
#' box()
#' axis(1,1:4,labels=c("2","4","6","8"))
